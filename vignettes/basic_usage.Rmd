...
The time series methods above assume that the outcome data has a continuous distribution. However, in reality we are forecasting _counts_, e.g., "how many influenza hospitalizations will occur next week in the United States?" In practice this does not matter when counts are large as the difference between a continuous and discrete distribution will have no meaningful effect on time series forecasts. However, when data contains small counts, methods specifically designed for forecasting _counts_ may have more desirable properties. Here we will fit count regression models forecasting influenza hospitalizations modeled on influenza-like illness (ILI) indicators and historical hospitalization severity for given epidemiological weeks, automatically finding the "best" models from possible covariates and model families (e.g., Poisson, Quasipoisson, Negative binomial, etc.).

### ILI retrieval and prep

First we must retrieve ILI data using the `get_cdc_ili()` function, which is a wrapper around `cdcfluview::ilinet()`. Here we're only using recent ILI data (i.e., since 2019). We next replace _all_ of Florida's weighted ILI data using Nowcast data from [CMU Delphi ILI Nearby](https://delphi.cmu.edu/nowcast/) (ILINet does not have weighted ILI data for Florida). We next replace the current week and one week previous with [Nowcast data](https://delphi.cmu.edu/nowcast/) to created augmented data (`iliaug`). Finally, we fit a time series model to forecast ILI for the next four weeks.

```{r, eval=FALSE}
ilidat <- get_cdc_ili(region=c("national"), years=2019:2022)
...
First we create a list of different count regression models to fit. Inspect the Poisson model formulation and the negative binomial model specification using an offset.
...
We must next project covariates four weeks into the future. Do this by pulling from the forecasted ILI and historical severity data.
...
Take a look at the model forecast as well as different aspects of the final fitted model.
...
Prepare the data for submission to flusight.
...
Optionally, validate the forecast according to Flusight's validation scheme.

```{r, eval=FALSE}
validate_forecast(us_glm_prepped$CREG)
```

Finally, visualize the forecast with the observed data.

```{r, warning=FALSE}
plot_forecast(dat_us, us_glm_prepped$CREG, location="US")
```

The count regression model appears to forecast far fewer influenza hospitalizations than recent weeks' data might suggest, and far fewer than the time series approach above forecasted. Remember that the count regression model is based on current ILI and historical influenza hospitalization severity. The 2022 season is atypical in that the season peaked in late spring, when flu hospitalizations had historically been low. The typical downturn in ILI and flu hospitalizations in early spring combined with the 2021-2022 season's atypically late peak is likely the cause of this low forecast.
