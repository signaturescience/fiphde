...
subform <- function(x) {
  ...
  loc <-
    fiphde:::locations %>%
    filter(abbreviation == x$location) %>%
    pull(location)
...
# Forecast ILI
ilifor_us <- forecast_ili(ilidat_us, horizon=5L, trim_date="2020-03-01", constrained=TRUE)
...
tmp_weekly_flu <-
  ...
  summarise(flu.admits = sum(flu.admits, na.rm = TRUE),
            flu.admits.cov = sum(flu.admits.cov, na.rm = TRUE),
            .groups = "drop") %>%
  left_join(ilifor_us$ilidat, by = c("epiyear", "epiweek")) %>%
  mutate(location = "US")
...
us_glm_prepped <-
  ...
  dplyr::select(forecast_date,target,target_end_date,location,type,quantile,value) %>%
  ## call to distinct to get rid of duplicated 0.5 quantile
  ## BETTER WAY TO DO THIS ??  dplyr::distinct()
  dplyr::distinct()
